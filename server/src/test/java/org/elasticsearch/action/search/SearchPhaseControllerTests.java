...
public class SearchPhaseControllerTests extends ESTestCase {
    ...
    private static AtomicArray<SearchPhaseResult> generateQueryResults(int nShards, List<CompletionSuggestion> suggestions,
        ...
        for (int shardIndex = 0; shardIndex < nShards; shardIndex++) {
            ...
            if (searchHitsSize == 0) {
                ...
                topDocs = new TopDocs(new TotalHits(scoreDocs.length, Relation.EQUAL_TO), scoreDocs);
            }
    ...
    public void testConsumer() {
        ...
        result.topDocs(new TopDocsAndMaxScore(new TopDocs(new TotalHits(0, Relation.EQUAL_TO), new ScoreDoc[0]), Float.NaN),
        ...
        for (int i = 0; i < expectedNumResults; i++) {
            ...
            threads[i] = new Thread(() -> {
                ...
                result.topDocs(new TopDocsAndMaxScore(
                    new TopDocs(new TotalHits(1, Relation.EQUAL_TO), new ScoreDoc[] {new ScoreDoc(0, number)}), number),
                    new DocValueFormat[0]);
    ...
    public void testConsumerOnlyAggs() {
        ...
        for (int i = 0; i < expectedNumResults; i++) {
            ...
            QuerySearchResult result = new QuerySearchResult(i, new SearchShardTarget("node", new ShardId("a", "b", i),
                null, OriginalIndices.NONE));
            result.topDocs(new TopDocsAndMaxScore(new TopDocs(new TotalHits(1, Relation.EQUAL_TO), new ScoreDoc[0]), number),
                    new DocValueFormat[0]);
            InternalAggregations aggs = new InternalAggregations(Collections.singletonList(new InternalMax("test", (double) number,
                DocValueFormat.RAW, Collections.emptyList(), Collections.emptyMap())));
            result.aggregations(aggs);
    ...
    public void testConsumerOnlyHits() {
        ...
        for (int i = 0; i < expectedNumResults; i++) {
            ...
            QuerySearchResult result = new QuerySearchResult(i, new SearchShardTarget("node", new ShardId("a", "b", i),
                null, OriginalIndices.NONE));
            result.topDocs(new TopDocsAndMaxScore(new TopDocs(new TotalHits(1, Relation.EQUAL_TO),
    ...
    public void testReduceTopNWithFromOffset() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            result.topDocs(new TopDocsAndMaxScore(new TopDocs(new TotalHits(3, Relation.EQUAL_TO), docs), docs[0].score),
                    new DocValueFormat[0]);
            result.setShardIndex(i);
            result.size(5);
            result.from(5);
