...
public class ShardFailedClusterStateTaskExecutorTests extends ESAllocationTestCase {
    ...
    public void testEmptyTaskListProducesSameClusterState() throws Exception {
        List<FailedShardEntry> tasks = Collections.emptyList();
        ClusterStateTaskExecutor.ClusterTasksResult<FailedShardEntry> result =
    ...
    public void testMarkAsStaleWhenFailingShard() throws Exception {
        ...
        {
            FailedShardEntry failShardOnly = new FailedShardEntry(shardRoutingTable.shardId(),
        ...
        }
        {
            ...
            FailedShardEntry failAndMarkAsStale = new FailedShardEntry(shardRoutingTable.shardId(),
    ...
    private List<FailedShardEntry> createExistingShards(ClusterState currentState, String reason) {
    ...
    private List<FailedShardEntry> createNonExistentShards(ClusterState currentState, String reason) {
        ...
        List<FailedShardEntry> existingShards = createExistingShards(currentState, reason);
        List<FailedShardEntry> shardsWithMismatchedAllocationIds = new ArrayList<>();
        for (FailedShardEntry existingShard : existingShards) {
            shardsWithMismatchedAllocationIds.add(new FailedShardEntry(existingShard.shardId,
        ...
        nonExistentShards.forEach(shard -> tasks.add(
            new FailedShardEntry(shard.shardId(), shard.allocationId().getId(), 0L,
    ...
    private static void assertTaskResults(
        List<Tuple<FailedShardEntry, ClusterStateTaskExecutor.TaskResult>> taskResultList,
        ClusterStateTaskExecutor.ClusterTasksResult<FailedShardEntry> result,
        ClusterState clusterState,
        boolean clusterStateChanged
    ) {
    ...
    private static List<FailedShardEntry> toTasks(ClusterState currentState, List<ShardRouting> shards,
                                                  String indexUUID, String message) {
        return shards
            .stream()
            .map(shard -> new FailedShardEntry(
                shard.shardId(),
