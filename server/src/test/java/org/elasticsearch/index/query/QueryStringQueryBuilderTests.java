...
public class QueryStringQueryBuilderTests extends AbstractQueryTestCase<QueryStringQueryBuilder> {
    ...
    public void testToQueryWildcardQuery() throws Exception {
        for (Operator op : Operator.values()) {
            Occur defaultOp = op.toBooleanClauseOccur();
    ...
    public void testToQueryWithGraph() throws Exception {
        for (Operator op : Operator.values()) {
            ...
            expectedQuery = new BooleanQuery.Builder()
                .add(new TermQuery(new Term(STRING_FIELD_NAME, "that")), Occur.MUST)
                .add(new TermQuery(new Term(STRING_FIELD_NAME, "guinea")), Occur.MUST_NOT)
    ...
    public void testToQueryBooleanQueryMultipleBoosts() throws Exception {
        ...
        assertThat(booleanQuery.clauses().get(0).getOccur(), equalTo(Occur.SHOULD));
        assertThat(booleanQuery.clauses().get(0).getQuery(),
            equalTo(new TermQuery(new Term(STRING_FIELD_NAME, "foo"))));
        assertThat(booleanQuery.clauses().get(1).getOccur(), equalTo(Occur.SHOULD));
    ...
    public void testToQueryTextParsing() throws IOException {
        {
            ...
            BooleanQuery bq1 =
                new BooleanQuery.Builder()
                    .add(new BooleanClause(new TermQuery(new Term(STRING_FIELD_NAME, "foo")), Occur.SHOULD))
                    .add(new BooleanClause(new TermQuery(new Term(STRING_FIELD_NAME, "bar")), Occur.SHOULD))
                    .build();
...

        {
            ...
            BooleanQuery expectedQuery =
                new BooleanQuery.Builder()
                    .add(disjunctionMaxQuery, Occur.SHOULD)
                    .add(new TermQuery(new Term(STRING_FIELD_NAME, "other")), Occur.SHOULD)
                    .build();
            assertThat(query, equalTo(expectedQuery));
        }

        {
            ...
            BooleanQuery expectedQuery =
                new BooleanQuery.Builder()
                    .add(new BooleanClause(maxQuery1, Occur.SHOULD))
                    .add(new BooleanClause(maxQuery2, Occur.SHOULD))
                    .build();
            assertThat(query, equalTo(expectedQuery));
        }
    ...
    public void testUnmappedFieldRewriteToMatchNoDocs() throws IOException {
        ...
        BooleanQuery expected = new BooleanQuery.Builder()
            .add(new TermQuery(new Term(STRING_FIELD_NAME, "first")), Occur.MUST)
    ...
    public void testWithStopWords() throws Exception {
        ...
        query = new BoolQueryBuilder()
            ...
            .toQuery(createShardContext());
        expected = new BooleanQuery.Builder()
            .add(new BooleanQuery.Builder().build(), Occur.SHOULD)
            .build();
        assertEquals(expected, query);
