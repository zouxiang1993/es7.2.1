...
public class BulkByScrollTaskStatusTests extends AbstractXContentTestCase<Status> {
    ...
    public void testBulkByTaskStatus() throws IOException {
        ...
        status.writeTo(out);
        Status tripped = new Status(out.bytes().streamInput());
     ...
     */
    public static void assertTaskStatusEquals(Version version, Status expected, Status actual) {
    ...
    public static Status randomStatus() {
        ...
        List<BulkByScrollTask.StatusOrException> statuses = IntStream.range(0, between(0, 10))
                ...
                .collect(toList());
        return new Status(statuses, randomBoolean() ? "test" : null);
    }

    public static Status randomStatusWithoutException() {
        if (randomBoolean()) {
            return randomWorkingStatus(null);
        }
        boolean canHaveNullStatues = randomBoolean();
    ...
    private static Status randomWorkingStatus(Integer sliceId) {
    ...
    public static void assertEqualStatus(Status expected, Status actual,
    ...
    @Override
    protected void assertEqualInstances(Status first, Status second) {
    ...
    @Override
    protected Status doParseInstance(XContentParser parser) throws IOException {
        return Status.fromXContent(parser);
    }
