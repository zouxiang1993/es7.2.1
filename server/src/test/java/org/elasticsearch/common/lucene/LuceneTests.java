...
public class LuceneTests extends ESTestCase {
    ...
    public void testWaitForIndex() throws Exception {
        ...
        doc.add(new TextField("id", "1", random().nextBoolean() ? Store.YES : Store.NO));
    ...
    public void testCleanIndex() throws IOException {
        ...
        doc = new Document();
        doc.add(new TextField("id", "2", random().nextBoolean() ? Store.YES : Store.NO));
        writer.addDocument(doc);

        doc = new Document();
        doc.add(new TextField("id", "3", random().nextBoolean() ? Store.YES : Store.NO));
        writer.addDocument(doc);

        writer.commit();
        doc = new Document();
        doc.add(new TextField("id", "4", random().nextBoolean() ? Store.YES : Store.NO));
        writer.addDocument(doc);
    ...
    public void testNumDocs() throws IOException {
        ...
        for (int i = 0; i < numDocsToIndex; i++) {
            doc = new Document();
            doc.add(new TextField("id", "extra_" + i, random().nextBoolean() ? Store.YES : Store.NO));
