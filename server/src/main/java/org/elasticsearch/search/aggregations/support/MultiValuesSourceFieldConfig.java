...
public class MultiValuesSourceFieldConfig implements Writeable, ToXContentObject {
    ...
    public static final BiFunction<Boolean, Boolean, ObjectParser<Builder, Void>> PARSER
        = (scriptable, timezoneAware) -> {

        ObjectParser<Builder, Void> parser
            = new ObjectParser<>(MultiValuesSourceFieldConfig.NAME, Builder::new);

        parser.declareString(Builder::setFieldName, ParseField.CommonFields.FIELD);
        parser.declareField(Builder::setMissing, XContentParser::objectText,
            ParseField.CommonFields.MISSING, ObjectParser.ValueType.VALUE);

        if (scriptable) {
            parser.declareField(Builder::setScript,
